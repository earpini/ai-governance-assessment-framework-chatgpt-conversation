...
1. Ettore: review the new front end on branch `frontend-v2` (build with `pnpm run build:pages`, preview with `pnpm exec vite preview --config vite.pages.config.ts`, open /brand.html), then merge and push so Pages redeploys.
2. Collector fix: OpenAlex type:article filter on T1 numerator AND denominator (JP artifact); GDELT resume-from-archive so successive monthly runs fill gaps instead of restarting.
3. Trends alpha application (Ettore); repo hygiene (duplicate files, retire old pipeline + PolicyWindowApp when confident).
